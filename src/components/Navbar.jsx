...
const Navbar = () => {
  ...
  return (
    <nav className='relative w-full bg-black text-white px-8 md:px-16 lg:px-24 z-50'>
      <div className='w-full mx-auto py-2 flex justify-between items-center'>
        ...
        <button className='hidden lg:inline bg-gradient-to-r from-green-400 to-blue-500 text-white transform transition-transform duration-300 hover:scale-105 px-4 py-2 rounded'>
          Connect me
        </button>
        <button className='lg:hidden bg-black text-white p-2 rounded' onClick={toggleMenu}>
          <svg className='w-6 h-6' fill='none' stroke='currentColor' viewBox='0 0 24 24' xmlns='http://www.w3.org/2000/svg'>
            <path strokeLinecap='round' strokeLinejoin='round' strokeWidth='2' d='M4 6h16M4 12h16m-7 6h7'></path>
          </svg>
        </button>
      </div>
      <div className={`lg:hidden fixed top-0 left-0 h-full w-full bg-black transition-transform transform ${isOpen ? 'translate-x-0' : '-translate-x-full'} z-50`}>
